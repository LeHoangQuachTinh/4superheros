...
public class ListCustomer implements IListAccount{
    ...
    public void lockAccount(String matk) {
        if(listCustomer.containsKey(matk))
        {
            listCustomer.get(matk).isLocked=true;
            System.out.printf("Đã khoá tài khoản ngừoi dùng :%s | %s",matk,listCustomer.get(matk).username);
        }
        else
