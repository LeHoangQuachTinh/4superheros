...
class Customer extends Account implements IDish {
    ...
    private HashMap<String,TaiKhoan> lienKetNganHang;
    ...
    public void muaHangTrucTiep(HashMap<String, Dish> menu, String tenMon, int soLuong) {
        // Tìm món ăn theo tên từ menu (HashMap)
    ...
    public void themLienKet(String stk,TaiKhoan taikhoan){
        if(!this.lienKetNganHang.containsKey(stk))
        {
            lienKetNganHang.put(stk, taikhoan);
            System.out.printf("\nThêm thành công tài khoản( %s , %s )vào Food\n",stk,taikhoan.getChutk());
        }
        else
            System.out.println("\nTài khoản này đã được liên kết hoặc đã liên kết với tài khoản khác.Vui lòng thử lại!\n");
    }
